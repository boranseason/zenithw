...
function render(){
  ...
  picker.innerHTML=UPDATES.map(x=>`<option value="${x.ver}"${x.ver===u.ver?' selected':''}>${x.ver}${x.latest?' · '+t.latestLabel:''}</option>`).join('');

  const newer=UPDATES[idx-1], older=UPDATES[idx+1];
  let nav='';
  if(newer)nav+=`<button type="button" class="upd-nav-link" onclick="jumpTo('${newer.ver}')"><span class="upd-nav-label">${t.newer}</span>← ${newer.ver}</button>`;
  else nav+=`<span aria-hidden="true"></span>`;
  if(older)nav+=`<button type="button" class="upd-nav-link next" onclick="jumpTo('${older.ver}')"><span class="upd-nav-label">${t.older}</span>${older.ver} →</button>`;
  else nav+=`<span aria-hidden="true"></span>`;
