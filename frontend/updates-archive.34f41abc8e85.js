...
sections:[
{hTr:'gizlilik politikası gerçek uygulamayla eşleşiyor',hEn:'privacy policy matches the real service',pTr:'Hesapsız kullanım, yerel geçmiş, geçici dosya ömrü, IP tabanlı korumalar, kaynak platformlar, altyapı sağlayıcıları ve analiz kullanımı ayrı başlıklarla açıklandı. Mutlak güvenlik vaadi yerine hangi katmanların riski nasıl azalttığı anlatılıyor.',pEn:'Account-free use, local history, temporary-file lifecycle, IP-based protections, source platforms, infrastructure providers, and analytics are now explained in separate sections. The policy describes how each layer reduces risk instead of promising absolute security.'},
