...
introTr:['Web arayüzü Netlify’dan Cloudflare Pages’e taşındı; zenithw.space, SSL ve statik dosyalar Cloudflare üzerinden sunulurken indirme ve dönüştürme backend’i aynı API adresinde kaldı.'],
introEn:['The web interface moved from Netlify to Cloudflare Pages. zenithw.space, SSL, and static assets are served by Cloudflare while the download and conversion backend remains on the same API address.'],
sections:[
...
{hTr:'önbellek ve API sınırı korundu',hEn:'cache and API boundaries preserved',pTr:'Sürümlü varlıklar uzun süreli önbelleği koruyor; HTML ve sürüm bilgisi yeniden doğrulanıyor. İndirme, remux ve dönüştürme işlemleri değişmeyen api.zenithw.space adresinden backend’e gidiyor.',pEn:'Versioned assets keep long-lived caching while HTML and version metadata revalidate. Downloads, remuxing, and conversion continue through the unchanged api.zenithw.space backend endpoint.'},
...
pTr:'EJS desteği sağlayan yt-dlp-ejs paketi artık production bağımlılık listesinde de yer alıyor. Daha önce bu paket eksik olduğu için sunucu tarafında da aynı 360p sorunu yaşanabiliyordu. Artık Linux sunucu ortamlarında da 4K doğrudan çalışıyor.',
pEn:'The yt-dlp-ejs package that enables EJS support is now included in the production dependency list as well. Previously its absence could cause the same 360p fallback on the server side. 4K now works out of the box in Linux server environments.'}
],
